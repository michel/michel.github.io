...
export const filesystem: Record<string, FSEntry> = {
	...
	"/etc/passwd": {
		...
		content: [
			...
			"michel:x:1337:1337:1337 h4x0r:/home/michel:/usr/bin/fish",
	...
	"/home/michel/.bashrc": {
		type: "file",
		content: [
			"# .bashrc - Why are you here?",
			"",
			"# If you somehow ended up in bash, let's fix that",
			"exec fish",
			"",
			"\x1b[comment]bash is for casuals. I use fish btw.\x1b[reset]",
		],
	},
	"/home/michel/.config": { type: "dir" },
	"/home/michel/.config/fish": { type: "dir" },
	"/home/michel/.config/fish/config.fish": {
		type: "file",
		content: [
			"# config.fish - Fast and Friendly",
			"# Load time: instant (as it should be)",
			"",
			"# No plugin managers needed - fish is batteries included",
			"",
			"# Aliases (abbr in fish)",
			"abbr -a please sudo",
			"abbr -a yolo 'git push --force'",
			"abbr -a vim nvim",
			"abbr -a nano nvim  # no escape",
			"",
			"# Greeting",
			...
			"end",
			"",
			"\x1b[comment]Terminal opens instantly.\x1b[reset]",
			"\x1b[comment]*Laughs in fast shell startup*\x1b[reset]",
		],
	},
	"/home/michel/.vimrc": {
		type: "file",
	...
	"/home/michel/.local/share/fish": { type: "dir" },
	"/home/michel/.local/share/fish/fish_history": {
		type: "file",
		content: [
			"- cmd: sudo rm -rf /",
			"  when: 1702345678",
			...
			"  when: 1702345683",
			"- cmd: why is node_modules so big",
			"  when: 1702345684",
			"- cmd: how to exit vim",
			"  when: 1702345685",
			"- cmd: curl http://localhost:3000 | grep error | wc -l",
			"  when: 1702345686",
			"- cmd: history delete --exact --case-sensitive ':(){ :|:& };:'",
			"  when: 1702345687",
			"",
			"\x1b[comment]My fish history is still a cry for help.\x1b[reset]",
		],
	},
	...
	// /usr
	"/usr/bin": { type: "dir" },
	"/usr/bin/cowsay": { type: "file" },
	"/usr/bin/sl": { type: "file" },
	"/usr/bin/lolcat": { type: "file" },
	"/usr/bin/vim": { type: "file" },
	"/usr/bin/nvim": { type: "file" },
	"/usr/bin/git": { type: "file" },
	"/usr/bin/curl": { type: "file" },
	"/usr/bin/wget": { type: "file" },
	"/usr/bin/htop": { type: "file" },
	"/usr/bin/tmux": { type: "file" },
	"/usr/bin/ssh": { type: "file" },
	"/usr/bin/rsync": { type: "file" },
	"/usr/bin/tar": { type: "file" },
	"/usr/bin/gzip": { type: "file" },
	"/usr/bin/make": { type: "file" },
	"/usr/bin/gcc": { type: "file" },
	"/usr/bin/fish": { type: "file" },
	"/usr/local": { type: "dir" },
	"/usr/local/bin": { type: "dir" },
	"/usr/local/bin/python3": { type: "file" },
	"/usr/local/bin/node": { type: "file" },
	"/usr/local/bin/npm": { type: "file" },
	"/usr/local/bin/cargo": { type: "file" },
	"/usr/local/bin/rustc": { type: "file" },
	"/usr/share": { type: "dir" },
