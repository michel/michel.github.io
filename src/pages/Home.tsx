...
const Line = ({ children }: { children: React.ReactNode }) => (
	<div className="flex">
		...
		<span className="flex-1 text-fg pl-5" style={{ width: `${BOX_CHARS}ch` }}>{children}</span>
		<span className="text-green font-bold">║</span>
	</div>
)
...
const InfoBox = () => {
	...
	const Cmd = ({ cmd, desc }: { cmd: string; desc: string }) => (
		<Line>
			{"    "}
			<span className="text-cyan cursor-pointer hover:underline" onClick={() => queueTerminalCommand(cmd)}>{cmd}</span>
			<span className="text-comment"> {".".repeat(Math.max(1, 10 - cmd.length))} {desc}</span>
		</Line>
	)
	...
	return (
		<div className="font-mono" style={{ width: `${BOX_CHARS + 2}ch` }}>
			...
			<Line> </Line>
			<Line>    <span className="text-cyan">Ctrl+p</span>   fuzzy search</Line>
			<Line>    <span className="text-cyan">Ctrl+`</span>   toggle terminal</Line>
			<Line>    <span className="text-cyan">j/k</span>      scroll up/down</Line>
			<Line>    <span className="text-cyan">:</span>        vim command line</Line>
			<Line> </Line>
			...
			<Line> </Line>
			<Line>  <span className="text-yellow">Easter eggs hidden everywhere. Have fun exploring!</span></Line>
			<Line> </Line>
			<div className="text-green font-bold">╚{"═".repeat(BOX_CHARS)}╝</div>
		</div>
	)
